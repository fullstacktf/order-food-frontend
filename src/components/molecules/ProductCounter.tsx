...
export const ProductCounter: FC<ProductCounterProps> = ({ updateTotalPrice }) => {
  ...
  return (
    <div className='product_counter flex_centered around'>
      <Button text="-" styles="counter_button" onClickHandler={handleClick(-1)}></Button>
      <input className='number_input' type="number" min="1" placeholder={quantity.toString()} value={inputValue} onChange={inputChange} />
      <Button text="+" styles="counter_button" onClickHandler={handleClick(1)}></Button>
    </div>
  )
}
